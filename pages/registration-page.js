...
class RegistrationPage {
  ...
  constructor(page) {
    ...
    this.successMessage = page.locator(
      "text=Cadastro realizado com sucesso, text=Sucesso, text=success",
    );
    //this.homePageIndicator = page.locator('text=BugBank, text=Home, text=Bem-vindo');
    this.alertSuccess = page.locator(
  ...
  async fillConfirmacaoSenha(confirmacaoSenha) {
    const registrationForm = this.getRegistrationForm();
    await registrationForm
      .locator('input[placeholder="Informe a confirmação da senha"]')
      .fill(confirmacaoSenha);
  }
  ...
  async getErrorMessageForField(fieldPlaceholder) {
    const registrationForm = this.getRegistrationForm();
    const fieldInput = registrationForm.locator(
  ...
  async hasSuccessMessage() {
    return (
      (await this.successMessage.count()) > 0 &&
  ...
  async hasAlertSuccess() {
    return (
      (await this.alertSuccess.count()) > 0 &&
